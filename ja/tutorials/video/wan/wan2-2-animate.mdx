...
6. 「Video Extend」グループは、出力ビデオの長さを延長するためのものです。
    ...
    - ワークフローには 2 つ目の `Video Extend` サブグラフがすでに含まれています。1 つ目と配線済みのままバイパスされているので、それと隣のバイパスされた `SaveVideo` に対して `Ctrl + B` を押すとさらに延長できます。複製や再配線は不要です
    - 3 段以上が必要な場合は、`Video Extend` サブグラフを自分で複製し、前の段の `IMAGE` 出力を新しい段の `image1` 入力へ、`video_frame_offset` 出力を新しい段の `video_frame_offset` 入力へ接続します
