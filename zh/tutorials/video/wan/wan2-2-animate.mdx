...
6. “Video Extend” 组用于扩展输出视频的长度
    ...
    - 工作流中已经包含第二个 `Video Extend` 子图，它与第一个子图已经连好线，只是处于旁路状态。对它以及它旁边被旁路的 `SaveVideo` 按 `Ctrl + B` 启用，即可继续延长输出，无需复制或重新连线
    - 如果你需要超过两段，请自行复制 `Video Extend` 子图，然后把上一段的 `IMAGE` 输出连接到新子图的 `image1` 输入，并把 `video_frame_offset` 输出连接到新子图的 `video_frame_offset` 输入
