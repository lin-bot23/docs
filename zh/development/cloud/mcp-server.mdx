...
<Warning>
  **封闭测试版——仅限邀请。** Comfy Cloud MCP目前处于封闭测试阶段，按用户功能标志进行访问控制。功能、工具和行为可能会随着项目发展而变化。
</Warning>

<CardGroup cols={2}>
  <Card title="加入等待列表" icon="clock" href="https://form.typeform.com/to/hHmvw1UH">
    还没有访问权限？立即注册 Comfy Cloud MCP 等待列表。
  </Card>
  <Card title="查看 GitHub 仓库" icon="github" href="https://github.com/Comfy-Org/comfy-cloud-mcp">
    浏览源代码、报告问题、查看最新更新。
  </Card>
</CardGroup>
...
**OAuth（[无需粘贴 API 密钥](/zh/development/api-development/getting-an-api-key)）** — 在 **Claude Code** 或 **Claude Desktop** 中添加远程服务器 `https://cloud.comfy.org/mcp`，使用客户端的 **Authenticate** 通过 Comfy 账号登录。连接成功后，合作节点生成与工作流工具用法相同。

[**API 密钥**](/zh/development/api-development/getting-an-api-key) — 仍完全支持。若客户端不支持 OAuth，或你更习惯使用密钥，请使用下方的一键安装或手动配置。

<Warning>
  **OAuth 逐步开放。** MCP 服务端已在生产环境支持 OAuth，但在下一次 Comfy Cloud 生产部署完成前（发现端点需同步更新），部分用户可能还无法完成端到端登录。若 OAuth 暂时不可用，请继续使用 [API 密钥](/zh/development/api-development/getting-an-api-key)——现有 API 密钥配置不受影响。
</Warning>

若使用 [API 密钥](/zh/development/api-development/getting-an-api-key)且尚未创建，请查看指引获取：
...
一键安装脚本会自动检测你的 MCP 客户端（Claude Code、Cursor、Amp），询问 [Comfy API 密钥](/zh/development/api-development/getting-an-api-key)并配置远程 MCP 服务器——无需 Node.js 或其他依赖。
...
Comfy Cloud MCP 托管在 `https://cloud.comfy.org/mcp`。将 MCP 客户端指向该 URL 并提供 [API 密钥](/zh/development/api-development/getting-an-api-key)：
...
| 类型 | 作用 |
|:-----|:-----|
| `/comfy-generate-image` | 从提示词生成图像 |
| `/comfy-generate-video` | 从文本或图像生成视频 |
| `/comfy-generate-3d` | 3D 模型（GLB/FBX/OBJ）|
| `/comfy-generate-audio` | 语音或音效 |
| `/comfy-upscale-image` | 放大图像 |
| `/comfy-remove-background` | 去除背景 |
| `/comfy-search-models` | 搜索模型目录 |
| `/comfy-search-nodes` | 搜索节点及接线提示 |
| `/comfy-search-templates` | 查找预构建工作流 |
| `/technique-combine-people` | 合成多人的合影 |
| `/comfy-help` | 查看帮助 |
...
**认证**
- **[API 密钥或 OAuth。](/zh/development/api-development/getting-an-api-key)** 手动配置使用 `X-API-Key` 头部的 [Comfy Cloud API 密钥](/zh/development/api-development/getting-an-api-key)。通过 Comfy OAuth 登录的 MCP 客户端会发送 `Authorization: Bearer` 令牌；托管服务器均支持。
