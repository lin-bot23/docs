...
6. For the "Video Extend" group, it's in order to extend to the output video length
    ...
    - The workflow already contains a second `Video Extend` subgraph. It is wired to the first one and bypassed, so press `Ctrl + B` on it and on the bypassed `SaveVideo` next to it to extend the output further: nothing has to be copied or rewired
    - If you need more than two segments, duplicate the `Video Extend` subgraph yourself, then connect the previous stage's `IMAGE` output to the new stage's `image1` input and its `video_frame_offset` output to the new stage's `video_frame_offset` input
